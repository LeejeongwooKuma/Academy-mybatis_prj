...
<title>JSP 템플릿</title>
<script src="http://192.168.10.76/mybatis_prj/common/js/color-modes.js"></script>

<link rel="shortcut icon"
	href="http://192.168.10.76/mybatis_prj/common/images/favicon2.ico" />
...
<link href="http://192.168.10.76/mybatis_prj/common/css/carousel.css"
	rel="stylesheet">
<c:import
	url="http://192.168.10.76/mybatis_prj/fragments/bootstrap_css.jsp" />
...
$(function(){
	$("#carCountry").change(function(){
		...
		$.ajax({
			...
			dataType:"JSON",
			error:function(xhr){
				alert("제조사 조회시 문제 발생. 잠시 후 다시 시도해주세요.");
			...
			success:function(jsonObj){
				if(jsonObj.result && jsonObj.makerCnt != 0){
					...
					for(var i=0; i < jsonObj.data.length; i++){
						selMaker.options[i+1]=new Option(jsonArr[i].maker);
					}//end for
				}//end if
			}
		});//ajax
	...
	$("#carMaker").change(function(){
		if($("#carMaker")[0].selectedIndex != 0){
				...
				$.ajax({
					...
					dataType:"JSON",
					error:function(xhr){
						alert("모델 조회시 문제 발생. 잠시 후 다시 시도해주세요.");
						console.log(xhr.status);
					},
					success:function(jsonObj){
	...
	$("#btn").click(function(){
		if($("#carModel")[0].selectedIndex != 0){
			...
			$.ajax({
				...
				dataType:"JSON",
				error:function(xhr){
					alert("세부차량 조회시 문제 발생. 잠시 후 다시 시도해주세요.");
					console.log(xhr.status);
				},
				success:function(jsonObj){
					if(jsonObj.result){
						...
						for(var i=0; i < jsonObj.data.length; i++){
							...
							createTr="<tr>";
							createTr+="<td><img src='http://localhost/mybatis_prj/day1226/car_img/"+ jsonObjCar.car_img 
...
<body>
	<header data-bs-theme="dark">
		<c:import url="http://192.168.10.76/mybatis_prj/fragments/header.jsp" />
	</header>
	<main>
  ...
  ================================================== -->
			...
			<div class="row featurette">
				<div>
					<div id="selectDiv">
						...
						</select> <input type="button" value="조회" id="btn" class="btn btn-info">
					</div>
					<div id="carListDiv">
		...
		<footer class="container">
			<c:import url="http://192.168.10.76/mybatis_prj/fragments/footer.jsp" />
		</footer>
	</main>
